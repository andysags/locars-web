...
export default async function PrivacyPage() {
  const privacyPath = join(process.cwd(), "Confidentialité Locars_01.05.2026.md");
  const privacyContent = await readFile(privacyPath, "utf8");
  const privacyContentWithoutTopTitle = privacyContent
    .replace(/^#\s+.+\r?\n?/, "")
    .trimStart();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      <main className="mx-auto max-w-6xl px-6 md:px-12 py-16 md:py-24">
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/5 p-8 sm:p-10 shadow-2xl shadow-black/20 backdrop-blur-sm">
          <LegalMarkdown content={privacyContentWithoutTopTitle} />
        </section>
      </main>
    </div>
  );
